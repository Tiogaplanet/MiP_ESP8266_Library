...
void MiP::sendIRDongleCode(uint8_t sendCode[], uint8_t dataNumbers, uint8_t transmitPower)
{
    ...
    MIP_ASSERT( dataNumbers == 0x02 || dataNumbers == 0x03 || dataNumbers == 0x04 );

    // Check for valid transmitPower values.  Must be between 1 and 120.
    MIP_ASSERT( 0x01 <= transmitPower && transmitPower <= 0x78 );

    // Check for valid codes in a three-byte send.
    if(dataNumbers == 0x03 && sendCode[3] != 0xFF)
        MIP_ASSERT( 0x00 <= sendCode[1] && sendCode[1] <= 0x03 );

    // Check for valid codes in a four-byte send.
    if(dataNumbers == 0x04 && sendCode[2] != 0xFF && sendCode[3] != 0xFF){
        MIP_ASSERT( 0x00 <= sendCode[0] && sendCode[0] <= 0x03 );
        MIP_ASSERT( 0x00 <= sendCode[1] && sendCode[1] <= 0x03 );
    }
    
    uint8_t dataBits;
    
    // MiP's API requires the usable data be specified in bits lengths of 0x10, 0x18 or 0x20.
    switch(dataNumbers)
    {
        case 0x02:
            dataBits = 0x10;
    ...
    // Send this command blindly with no error checking since there is no easy way to determine if it has failed.
    rawSendIRDongleCode(sendCode, dataBits, transmitPower);
}
