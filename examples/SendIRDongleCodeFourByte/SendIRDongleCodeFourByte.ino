...
uint32_t dongleCode = 0xFFFFFFFF;
...
void loop() {
  do {
    ...
    char formattedOutput[18];

    sprintf(formattedOutput, "Sending 0x%08X", dongleCode);

    Serial1.println(formattedOutput);

    mip.sendIRDongleCode(dongleCode, 0x20, MIP_IR_TX_POWER);

    delay(100);
  } while (dongleCode != 0xFFFFFFFF);
}
